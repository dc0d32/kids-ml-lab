"""Chapter 11 workbook · One Neuron."""
...
WORKBOOK = Workbook(
    ...
    intro='Use w1 = 2, w2 = -1, and b = 0.5. Work out the raw score first. Then squish it.',
    questions=[
        Question(
            prompt='For point (1, 0), what is the raw score z?',
            kind='number', answer=2.5, tolerance=0.001, table=POINTS,
            hint='2*1 - 1*0 + 0.5',
            why='The raw score is the Chapter 2 line inside the neuron: 2*1 - 0 + 0.5 = 2.5. The squish happens after this number is built.',
        ),
        Question(
            ...
            why='A sigmoid output below 0.5 lands on the blue side. The squish turns the line score into confidence, but it keeps the same boundary at z = 0.',
        ),
        Question(
            prompt='For point (1, 2), what is z?',
            kind='number', answer=0.5, tolerance=0.001,
            why='2*1 - 2 + 0.5 = 0.5. Positive means the point is on the red side, and the small size means it is near the fence.',
        ),
        Question(
            prompt='Now double w1, w2, and b. What is the new z for point (1, 2)?',
            kind='number', answer=1.0, tolerance=0.001,
            why='Every raw score doubles: 2*(0.5) = 1.0. The confidence changes because the point looks farther from the fence.',
        ),
        Question(
            ...
            why='The boundary is where z = 0. Multiplying the whole score by 2 keeps the same places at zero, so confidence changes while the line stays put.',
        ),
        Question(
            prompt='What weights make a neuron maximally unsure everywhere?',
            kind='text', answer=['w1=0 w2=0 b=0', '0 0 0', 'all zeros', 'w=0 b=0'],
            why='If all weights and the bias are zero, every point has z = 0 and sigmoid(0) = 0.5. The neuron has no reason to prefer either side.',
        ),
    ],
