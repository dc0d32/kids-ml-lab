...
WORKBOOK = Workbook(
    ...
    questions=[
        Question(
            ...
            hint='2*1 - 1*0 + 0.5',
            why='The raw score is the Chapter 2 line hiding inside the neuron.',
        ),
        Question(
            ...
            why='A sigmoid output below 0.5 lands on the blue side. The squish turns a line score into confidence.',
        ),
        Question(
            prompt='For point (1, 2), what is z?',
            kind='number', answer=0.5, tolerance=0.001,
            why='2*1 - 2 + 0.5 = 0.5. Positive means the point is on the red side, but not by much.',
        ),
        Question(
            prompt='Now double w1, w2, and b. What is the new z for point (1, 2)?',
            kind='number', answer=1.0, tolerance=0.001,
            why='Every raw score doubles. Points do not cross the boundary because zero doubled is still zero.',
        ),
        Question(
            ...
            why='The boundary is where z = 0. Multiplying the whole score by 2 keeps the same places at zero. Confidence changes; the line does not.',
        ),
        Question(
            prompt='What weights make a neuron maximally unsure everywhere?',
            kind='text', answer=['w1=0 w2=0 b=0', '0 0 0', 'all zeros', 'w=0 b=0'],
            why='If all weights and the bias are zero, every point has z = 0 and sigmoid(0) = 0.5.',
        ),
    ],
