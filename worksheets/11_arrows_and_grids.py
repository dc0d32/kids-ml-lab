...
WORKBOOK = Workbook(
    ...
    questions=[
        Question(
            ...
            kind="number",
            answer=length_6_8,
            why="`[6, 8]` is the 3-4-5 triangle doubled. Its length is `sqrt(6² + 8²) = sqrt(36 + 64) = sqrt(100) = 10`.",
        ),
        Question(
            prompt="Compute the dot product `[2, 3] · [4, 1]`.",
            kind="number",
            answer=dot_small,
            why="Multiply matching parts, then add: `2×4 + 3×1 = 8 + 3 = 11`. A positive dot product means the arrows have some same-way agreement.",
        ),
        Question(
            prompt="Compute the dot product `[2, 0] · [0, 5]`.",
            kind="number",
            answer=dot_right_angle,
            why="`2×0 + 0×5 = 0`. These arrows meet at a right angle, so neither arrow points along the other one.",
        ),
        Question(
            prompt="Apply `[[2, 0], [0, 3]]` to `[1, 1]`. What is the new x value?",
            kind="number",
            answer=stretched_result[0],
            why="The first row builds the new x value: `2×1 + 0×1 = 2`. The matrix doubles sideways movement.",
        ),
        Question(
            prompt="Apply `[[2, 0], [0, 3]]` to `[1, 1]`. What is the new y value?",
            kind="number",
            answer=stretched_result[1],
            why="The second row builds the new y value: `0×1 + 3×1 = 3`. The matrix triples up-down movement.",
        ),
        Question(
            prompt="Under `[[3, -1], [2, 4]]`, where does `(1, 0)` land? Give the x value.",
            kind="number",
            answer=float(first_column[0]),
            why="The arrow `(1, 0)` lands on the first column of the matrix. Here that column is `[3, 2]`, so the x value is 3.",
        ),
        Question(
            prompt="Under `[[3, -1], [2, 4]]`, where does `(1, 0)` land? Give the y value.",
            kind="number",
            answer=float(first_column[1]),
            why="Keep reading the first column: `[3, 2]`. The y value is 2.",
        ),
        Question(
            prompt="What is the area multiplier of `[[2, 0], [0, 3]]`?",
            kind="number",
            answer=area_stretch,
            why="A 1-by-1 square becomes 2 wide and 3 tall, so its area becomes `2×3 = 6`. The determinant says the same thing.",
        ),
        Question(
            prompt="What is the area multiplier of `[[1, 2], [2, 4]]`?",
            kind="number",
            answer=area_flat,
            why="The second column is twice the first, so both starter arrows land on the same line. The square collapses to a line, and a line has area 0.",
        ),
        Question(
            prompt="Multiply `[[2, 1], [0, 3]] @ [[0, 1], [1, 0]]`. What is the top-left number?",
            kind="number",
            answer=combined[0][0],
            why="Top row with left column: `2×0 + 1×1 = 1`. Matrix multiplication is a tidy way to fuse two grid moves.",
        ),
        Question(
            prompt="For that same product, what is the top-right number?",
            kind="number",
            answer=combined[0][1],
            why="Top row with right column: `2×1 + 1×0 = 2`. This tells where the second starter arrow lands after both moves.",
        ),
        Question(
            prompt="For that same product, what is the bottom-left number?",
            kind="number",
            answer=combined[1][0],
            why="Bottom row with left column: `0×0 + 3×1 = 3`.",
        ),
        Question(
            prompt="For that same product, what is the bottom-right number?",
            kind="number",
            answer=combined[1][1],
            why="Bottom row with right column: `0×1 + 3×0 = 0`. The combined matrix is `[[1, 2], [3, 0]]`.",
        ),
        Question(
            prompt="Why would stacking ten layers with no squish be a waste of time?",
            kind="open",
            why="Because all ten linear moves can be multiplied into one combined matrix. Without a squish, extra layers do not add new bending power; they are a longer way to say one linear move.",
        ),
    ],
    kid_corner=(
