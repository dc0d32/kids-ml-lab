...
from Chapter 5, random forests and boosting from Chapter 6, support vector machines from
Chapter 7, and nearest neighbours from Chapter 8. Which one should you use?
...
"""
    ...
    with knobs:
        shape = ui.shape_picker(default="moons", key="ch09_zoo_shape")
        noise = ui.noise_slider(default=0.20, key="ch09_zoo_noise")
        n = ui.sample_slider(default=180, key="ch09_zoo_n")
        seed = ui.seed_slider(default=0, key="ch09_zoo_seed")
...
"""
    ...
    lesson.look_for("the gap between train score and test score. The training score is the fake trophy: high, shiny, and no proof the model learned anything about new rows.")
    lesson.careful(
...
def _():
    guess = lesson.predict(
        ...
        key="ch09_split_bounce",
    )
    if guess is None:
        return

    lesson.say("The model type and dataset stay the same; only the split seed changes.")
    test_size = st.slider("How much data goes in the test set?", 0.15, 0.50, 0.30, 0.05, key="ch09_test_size")
