"""Chapter 10 · Real Data, Real Mess."""
...
def _():
    choice = st.selectbox("Pick a bundled table", TABLES, format_func=dataset_label, key="ch10_explorer_table")
...
"""
    ...
    guess = lesson.predict(
        ...
        key="ch10_feature_hunch",
    )
    if guess is None:
        return

    lesson.say("Pick the columns your model is allowed to use. You draft a team of clues, then the bar chart shows which clues the model actually leaned on.")
    draft_table = st.selectbox("Draft dataset", TABLES, index=2, format_func=dataset_label, key="ch10_draft_table")
    ...
    selected = st.multiselect("Your feature team", options, default=default_features, key="ch10_feature_team")
    if not selected:
...
"""
    ...
    draft_table = st.selectbox("Feature-importance dataset", TABLES, index=2, format_func=dataset_label, key="ch10_importance_table")
    df = datasets.load_table(draft_table)
    target = datasets.target_of(draft_table)
    options = [c for c in df.columns if c != target]
    default_features = realdata.suggested_features(draft_table)
    selected = st.multiselect("Feature team", options, default=default_features, key="ch10_importance_features")
    if not selected:
        st.warning("Pick at least one column so the model has something to look at.")
        return
...
def _():
    ...
    draft_table = st.selectbox("Leaderboard dataset", TABLES, index=2, format_func=dataset_label, key="ch10_board_table")
    df = datasets.load_table(draft_table)
    target = datasets.target_of(draft_table)
    options = [c for c in df.columns if c != target]
    selected = st.multiselect("Leaderboard feature team", options, default=realdata.suggested_features(draft_table), key="ch10_board_features")
    if not selected:
        st.warning("Pick at least one column so the model has something to look at.")
        return

    result = cached_train(draft_table, tuple(selected))
    if "ch10_board" not in st.session_state:
        st.session_state.ch10_board = []
    if st.button("Add this attempt to my leaderboard", type="primary", key="ch10_add_board"):
        st.session_state.ch10_board.append(
            {
                "dataset": draft_table,
    ...
    if st.session_state.ch10_board:
        board = pd.DataFrame(st.session_state.ch10_board).sort_values("score", ascending=False)
...
"""
    ...
    ax.legend()
    lesson.show(fig)
    lesson.look_for("points far from the dashed perfect line. Those are the days the model got most wrong, and they are the ones worth explaining.")
