...
def train_torch(shape: str, hidden: int, lr: float, epochs: int, seed: int):
    X, y = toy_shape(shape, n=180, noise=0.18, seed=seed)
    model = tb.mlp([2, hidden, 1], activation="tanh", seed=seed)
    result = tb.train(model, X, y, epochs=epochs, lr=lr)
...
""",
    ...
    lesson.say("If PyTorch remembers the forward recipe, it can walk that recipe backward and fill in gradients for every weight. Chapter 14, at framework speed!")
...
def _():
    lesson.say(
        """
The most useful knob here is not the shape or the width. It is **how many training steps**
you take. Each step is one downhill nudge, so more steps means the network has more chances
to fit the data.

Start at **20 steps**: the loss is still high and the boundary barely separates the two
colours, because the network has hardly moved yet. Slide up to a few hundred and the loss
drops and the boundary snaps into shape. This is why training takes patience, not magic.
"""
    )
    knobs, picture = lesson.controls()
    with knobs:
        shape = st.selectbox("Dataset", ["moons", "xor", "circles"], index=0, key="ch17_shape")
        hidden = st.slider("Hidden neurons", 2, 8, 3, key="ch17_hidden")
        lr = st.slider("PyTorch learning rate", 0.02, 1.0, 0.25, 0.02, key="ch17_lr")
        epochs = st.select_slider(
            "Training steps",
            options=[20, 50, 100, 200, 400, 800, 1200],
            value=400,
            key="ch17_epochs",
        )
    with picture:
        X, y, torch_model, losses, seconds = train_torch(shape, hidden, lr, epochs, 5)
        cols = st.columns(2)
        with cols[0]:
        ...
        with cols[1]:
            fig, ax = lesson.figure(5.2, 4.3)
            loss_curve(losses, ax=ax, title=f"{epochs} steps in {seconds:.2f}s")
            lesson.show(fig)
    lesson.look_for("the training-steps knob. At 20 the loss curve is short and high and the boundary is a blur; raise it and watch the curve slide down while the boundary sharpens.")
    guess = lesson.predict(
