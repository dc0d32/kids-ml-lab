...
""",
    ...
    lesson.jargon(
        "the forward pass, the loss, and the gradient",
        "The forward pass is the left-to-right trip that turns inputs into an output. The "
        "loss is the single number for how bad that output was. The gradient is which way "
        "to nudge each learned number to make the loss smaller.",
    )
...
def _():
    ...
    with knobs:
        lr = st.slider("Learning rate", 0.0, 8.0, 0.8, 0.1, key="ch14_lr")
        seed = st.slider("Random start", 0, 10, 2, 1, key="ch14_seed")
...
def _():
    knobs, picture = lesson.controls()
    with knobs:
        lr = st.slider("Loss learning rate", 0.0, 8.0, 0.8, 0.1, key="ch14_loss_lr")
        seed = st.slider("Loss random start", 0, 10, 2, 1, key="ch14_loss_seed")
...
def _():
    lesson.say(
        "Same downhill idea, now drawn on a map of the loss. Every dot is one step. To make "
        "this a map we can draw, we hold the bias still and let only the two weights move, so "
        "the loss really is a 2D valley. Darker is lower loss; the pink star is the bottom."
    )
    knobs, picture = lesson.controls()
    with knobs:
        lr = st.slider("Walk learning rate", 0.0, 8.0, 0.8, 0.1, key="ch14_walk_lr")
        seed = st.slider("Walk random start", 0, 10, 2, 1, key="ch14_walk_seed")
    with picture:
        fig, ax = lesson.figure(5.8, 4.6)
        draw_weight_walk(ax, lr, seed)
        lesson.show(fig)
    lesson.look_for("the dots. A tiny rate leaves them bunched near the start, still crawling. A good rate lands them on the star at the bottom. A big rate flings the walk past the star and up the far wall.")
    lesson.careful("Downhill finds the bottom it can reach from where it starts. A different start slides in from a different direction, but this valley has one bottom.")
...
def _():
    lesson.say(
        """
Here is the Chapter 13 wall with learning switched on. XOR still has the wrong shape
...
"""
    ...
    lesson.say("Chapter 15 changes the model, not the downhill idea. Same hill-walking engine, sharper vehicle!")
