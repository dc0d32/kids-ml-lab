"""Chapter 11 · Where Models Go Wrong."""
...
"""
    ...
    threshold = st.slider("How worried must the model be before it says sick?", 0.00, 1.00, 0.50, 0.05, key="ch11_threshold")
...
def _():
    ...
    lesson.look_for("the suspicious jump when leaked columns are allowed. A model that leaps to a near-perfect score on messy data is usually cheating, not learning.")
...
A model has no alarm for "this point is nothing like what I trained on." Chapter 00 warned you: a model answers anyway.
"""
    )
    span = st.slider("How far outside the moon world should we look?", 3.0, 10.0, 8.0, 1.0, key="ch11_far_span")
    far = cached_far(span)
...
3. Take the Chapter 10 monsters model and find a monster it is confidently wrong about.
