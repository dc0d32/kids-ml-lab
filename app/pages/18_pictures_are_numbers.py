"""Chapter 18 · Pictures Are Numbers."""
...
""",
    ...
    lesson.careful(
        "A plain MLP does not know that pixel 10 touches pixel 11. To it, stepping from pixel 10 to pixel 11 is no more special than teleporting from pixel 10 to pixel 47. Chapter 19 bolts the neighbour map back on."
    )
...
def _():
    ...
    lesson.say("Your eyes can read that as a **3** because the bright pixel numbers build a shape. The model does not see the shape. It keeps one separate weight — a tuning number it can turn up or down — for every one of the pixels, and has to learn all of them.")
...
def _():
    lesson.say("The canvas is a black drawing pad. Your mouse draws thick white strokes; the app crops the marks, shrinks them to 8 by 8, and converts brightness onto the same 0–16 number scale as the dataset. Keep the digit large and centred, because the training digits were neat and centred too. Drew a bad one? Hit the clear button under the pad and start fresh.")
    ...
    with left:
        if st_canvas is None:
            ...
            if "ch18_canvas_gen" not in st.session_state:
                st.session_state["ch18_canvas_gen"] = 0
            canvas = st_canvas(
                ...
                key=f"ch18_digit_canvas_{st.session_state['ch18_canvas_gen']}",
            )
            grid = vision.canvas_to_digit_grid(canvas.image_data)
            if st.button("🗑️ Clear the pad and start over", key="ch18_clear_canvas", type="primary"):
                st.session_state["ch18_canvas_gen"] += 1
                st.rerun()
    with right:
