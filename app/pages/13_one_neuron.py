...
@lesson.step("A neuron is a line plus a squish", beat="hook")
def _():
    lesson.say(
        """
Part 3 is called **neural networks**, which sounds like a whole new thing to learn. It is
not. A network is a bunch of *neurons* wired together, and one neuron is built out of two
pieces you already have.

Piece one is the straight-line score from Chapter 2: `w1*x1 + w2*x2 + b`. Piece two is the
squish from Chapter 4 that bends that score to land between 0 and 1. With **w1 = 2,
w2 = -1, b = 0.5** and the point **(1, 0)**, the score is `2*1 + (-1)*0 + 0.5 = 2.5`, and
the squish turns `2.5` into about `0.92` — pretty sure it is red.

So why add the squish? Without it a neuron is only a straight line, and Chapter 12 showed
that stacking straight lines gives you back one straight line. The squish bends each piece,
and bent pieces can stack into curves. That bend is why a stack of neurons can do more than
a single line ever could.
"""
    )
    lesson.mermaid(
        """
graph TD
    X1[x1] --> M[multiply by the weights]
    X2[x2] --> M
    M --> S[add them up, plus b]
    S --> A[squish]
    A --> Y[answer between 0 and 1]
""",
    )
    lesson.look_for("the two boxes in the middle. Multiplying and adding is the same line machine you already built; the squish is the only new box.")
    lesson.say("Read every arrow as `this number flows into the next box`. The two inputs `x1` and `x2` get multiplied by the weights, those pieces get added up together with the bias `b`, the squish bends that score to sit between 0 and 1, and that is the neuron's answer.")
...
def _():
    lesson.say("Drag the learned numbers and watch the green line. That line is where `z = 0`, the fence between the two answers.")
    knobs, picture = lesson.controls()
    with knobs:
        w1 = st.slider("w1", -6.0, 6.0, 2.0, 0.2, key="ch13_w1")
        w2 = st.slider("w2", -6.0, 6.0, -1.0, 0.2, key="ch13_w2")
        b = st.slider("b", -4.0, 4.0, 0.5, 0.1, key="ch13_b")
        activation = st.selectbox("Squish", list(ACTIVATIONS), index=list(ACTIVATIONS).index("sigmoid"), key="ch13_activation")
...
def _():
    ...
    knobs, picture = lesson.controls()
    with knobs:
        w1 = st.slider("surface w1", -6.0, 6.0, 2.0, 0.2, key="ch13_surface_w1")
        w2 = st.slider("surface w2", -6.0, 6.0, -1.0, 0.2, key="ch13_surface_w2")
        b = st.slider("surface b", -4.0, 4.0, 0.5, 0.1, key="ch13_surface_b")
        activation = st.selectbox("Surface squish", list(ACTIVATIONS), index=list(ACTIVATIONS).index("sigmoid"), key="ch13_surface_activation")
    X, _ = toy_shape("blobs", n=180, noise=0.22, seed=4)
    play_neuron = Neuron(w=np.array([w1, w2]), b=b, activation=activation)
    with picture:
...
def _():
    ...
    knobs, picture = lesson.controls()
    with knobs:
        w1 = st.slider("XOR w1", -6.0, 6.0, 2.0, 0.2, key="ch13_xor_w1")
        w2 = st.slider("XOR w2", -6.0, 6.0, -1.0, 0.2, key="ch13_xor_w2")
        b = st.slider("XOR b", -4.0, 4.0, 0.5, 0.1, key="ch13_xor_b")
...
Chapter 14 opens that training rule and shows how the knobs move.
"""
    )
