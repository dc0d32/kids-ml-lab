...
@st.cache_resource(show_spinner="Warming up your Chapter 23 model...")
...
def _():
    ...
    lesson.show(fig)
    lesson.look_for("the green line to Chapter 23, and the red branch that led through clustering and PCA.")
...
def _():
    ...
    choice = st.selectbox("Pick a real-world thing", list(uses), key="ch24_real_use")
    st.info(uses[choice])
...
def _():
    guess = lesson.predict(
        "Your Chapter 23 model never learned facts. If we start a factual sentence, what should we expect?",
    ...
    prompt = st.text_input("Start with a factual-looking prompt", value="the moon is made of ", key="ch24_hallucination_prompt")
    temperature = st.slider("Temperature", 0.05, 1.8, 0.9, 0.05, key="ch24_hallucination_temp")
    seed = st.slider("Random seed", 0, 99, 7, key="ch24_hallucination_seed")
    made = generate_transformer(bundle, start=prompt, temperature=temperature, length=180, seed=seed)
    st.text_area("Your tiny model continues", made, height=140, key="ch24_hallucination_text")
